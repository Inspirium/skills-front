...
<template>
  <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
    ...
    <div class="flex justify-between text-white mt-12">
      <div class="">
        <h1 class="font-semibold sm:text-[6rem] font-dosis">
          Istraži podgrupe vještine
        </h1>
      ...
      <p class="max-w-xl text-right">
        <span class="text-cyan-400 font-bold text-2xl font-dosis">Poljoprivreda, prehrana i veterina </span> prikazuju grupe sličnih specijalističkih zadataka. Specijalistički zadaci osmišljeni su tako da opisuju svakodnevni rad unutar zanimanja. Ovi zadaci su uglavnom prenosivi – ako možete obaviti jedan zadatak u klasteru, možete obaviti i ostale.
      </p>
    </div>
    <div class="flex flex-wrap mt-20">
      <div v-for="(item, index) in sectors" :key="index" :class="[hover === index ? 'activecell bounce' : 'bounce-reverse', index === 0 || index === 10 || index === 20 ? 'ml-[6.8rem]' : '']" class="-mr-[3.2rem] w-[270px] h-[240px] relative -mb-[3.2rem] cursor-pointer transition sace" @mouseover="hover = index" @mouseleave="hover = null">
