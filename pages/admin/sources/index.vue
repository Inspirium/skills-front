...
function loadIt() {
  useApiFetch('/api/v1/sources', {
    ...
    dataFetched.value = $jsonSerializer.deserialize('sources', data.data.value)
  })
}
loadIt()
...
function createIt() {
  useApiFetch('/api/v1/sources', {
    method: 'post',
    body: $jsonSerializer.serialize('sources', data.value),
  }).then(() => {
    loadIt()
  ...
  <div class="bg-indigo-600">
    <div class="h-28 max-w-7xl mx-auto py-8 px-4 sm:px-6">
      <h2 class="text-2xl font-semibold tracking-tight text-white sm:text-4xl">
        <span class="block">Unesi novi izvor vještine</span>
      </h2>
    </div>
  </div>
  <div class="px-8 bg-gray-100 py-10">
    <div class="block text-gray-700 font-bold text-xl mb-2">
      <div class="text-3xl mb-3">Izvor vještine</div>
