...
function createIt() {
  useApiFetch('/api/v1/skill-levels', {
    method: 'post',
    body: $jsonSerializer.serialize('skill-levels', data.value),
  }).then(() => {
    loadIt()
...
function deleteIt(id) {
  useApiFetch(`/api/v1/skill-levels/${id}`, {
    method: 'delete',
  }).then((data) => {
    loadIt()
    console.log(data)
  ...
  <div class="bg-indigo-600">
    <div class="h-28 max-w-7xl mx-auto py-8 px-4 sm:px-6">
      <h2 class="text-2xl font-semibold tracking-tight text-white sm:text-4xl">
        <span class="block">Unesi novu razinu vještine</span>
      </h2>
    </div>
  </div>
