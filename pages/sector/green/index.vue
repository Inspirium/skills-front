  ...
  <div v-if="skills.length" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex mt-12">
      ...
      <div class="w-full mb-10">
        <Search :placeholder="'Pretraži zelene vještine'" class="mb-4" @update="getDataSearch($event)" @reset="results = []" />
        <div v-for="(item, index) in results.length ? results : skills" class="font-dosis sm:ml-16 cursor-pointer group test" @click="router.push(`/sector/skill/${item.id}`)">
          ...
          <h2 class="text-3xl font-semibold pt-3 group-hover:scale-105 origin-left transition text-lime-600">
            {{ item.name }}
          </h2>
