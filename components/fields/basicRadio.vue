  ...
  <div class="sm:col-span-6">
    <div>
      <div class="flex items-center justify-between">
        <h2 class="text-md font-medium text-gray-900 -mb-1">{{ label }}</h2>
      </div>
      <RadioGroup v-model="selectedOption" class="mt-2">
        ...
        <div class="grid grid-cols-3 gap-3 sm:grid-cols-3">
          <RadioGroupOption as="template" v-for="option in options" :key="option.name" :value="option" v-slot="{ active, checked }">
