...
<template>
  <div v-if="lineBreak > 2" class="flex-wrap mt-12 flex">
    <div v-if="sectors.length" v-for="(item, index) in sectors" :key="index" :class="[hover === index ? 'activecell bounce' : 'bounce-reverse', index % lineBreak ? '' : 'ml-[6.8rem]']" class="-mr-[3.2rem] w-[270px] h-[240px] relative -mb-[3.2rem] opacity-0 scale-50 translate-y-[100px] cursor-pointer transition sace" @click="router.push(`/sector/${item.id}`)" @mouseover="hover = index" @mouseleave="hover = null">
      ...
      <font-awesome-icon v-if="item" class="absolute hex-icon w-10 h-10 " :color="hover !== index ? `#${item.color}` : '#ffffff'" :icon="item.icon" />
      <svg
  ...
  <div v-if="lineBreak <= 2" class="flex flex-col mt-12">
    <div v-if="sectors.length" v-for="(item, index) in sectors" :key="index" :class="[index % 2 ? '-mt-4' : 'ml-[2rem] -mt-4']" class="h-[120px] relative opacity-0 scale-50 translate-y-[100px] cursor-pointer transition sace flex items-center relative" @click="router.push(`/sector/${item.id}`)">
